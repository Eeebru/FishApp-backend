...
router.post("/", async function (req, res, next) {
  const query = {
    text:
      "INSERT INTO fishes (name, type, image_url, description) VALUES($1, $2, $3, $4) RETURNING *",
    values: [
      req.body.name,
      req.body.type,
      req.body.image_url,
  ...
  try {
    const data = validate(req.body, jsonData);
...
router.patch("/:id", async (req, res, next) => {
  const query = {
    text:
      "UPDATE fishes SET name = $1, type = $2, image_url = $3, description = $4 WHERE id = $5 RETURNING *",
    values: [
      req.body.name,
      req.body.type,
      req.body.image_url,
